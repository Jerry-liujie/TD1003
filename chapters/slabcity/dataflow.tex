...
SQL offers distinct constructs with the same underlying semantics: a
comparison $\dataflowchain_1 \leq \dataflowchain_2$ and an ordering
$\sqlorderbycolor$ over the compared dataflows both express that the
computation is sensitive to the order of those values, and
$\sqlgroupbycolor \ \dataflowchain$ and $\sqlpartitionbycolor \
\dataflowchain$ both express grouping by $\dataflowchain$, differing
only in whether the groups are collapsed or kept. Two queries that
make opposite choices among such constructs perform the same essential
computation while manifesting literally different dataflow terms;
$\query_1$ and $\query_2$ from the running example are exactly such a
pair. We therefore define pairs of dataflow forms with shared
semantics to be \emph{alternatives} of one another; the ordering and
grouping pairs above are two such families, and our implementation
recognizes further families of the same character. When extracting the
...
merely for expressing the input's computation through a semantically
equal construct; candidates' own sets are left literal, so the closure
...
enables efficient search, was made jointly with Rui Dong.
