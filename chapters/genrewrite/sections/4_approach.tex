...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
  \Require{\parbox[t]{\dimexpr\linewidth-6em\relax}{$\mathcal{Q}$: queries, $\mathcal{L}$: LLM, $\mathcal{D}$: database, $\mathcal{T}$: tester, $B$: budget, $\theta$: user-specified minimum desirable speedup}}
  \Ensure{\parbox[t]{\dimexpr\linewidth-6em\relax}{$\mathit{Res} = \{\langle q, q', e\rangle | \forall q \in \mathcal{Q}_{opt} \subseteq \mathcal{Q}$, where $\mathcal{Q}_{opt}$ is the optimizable subset of $\mathcal{Q} \}$: Optimized queries \& associated NLR2s}}
