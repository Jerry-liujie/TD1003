...
SlabCity~\cite{dong2023slabcity} turns this idea into the first
synthesis-based technique capable of whole-query optimization without
any rewrite rules. Its
...
GenRewrite~\cite{genrewrite} is the first holistic system to make
language models dependable for query rewriting, and its central idea is to let rules
...
DAGSmith~\cite{dagsmith-arxiv} is built on the observation that those
dependencies are not merely execution constraints but optimization
signals. Reading the
...
whole-query SQL optimization that requires no rewrite rules. The
author's contribution is the query-dataflow abstraction: its
definition, its extraction from SQL, and the observation that related
queries can be connected through shared dataflows. Rui Dong developed
the dataflow-driven scoring function and search algorithm that make
rule-free synthesis efficient. The system's layered equivalence
checking and its evaluation on four workloads were collaborative
efforts, as was the curation of a benchmark of more than 1{,}000
real-life queries released with the system. SlabCity optimizes more queries than state-of-the-art
rule-based rewriters, discovers rewrites that no composition of their
rules can express, and produces rewrites that are often several times
faster than the original queries.
...
present SlabCity, GenRewrite, and DAGSmith. \Cref{chap:related}
surveys related work across the three systems,
\cref{chap:future} discusses directions for future work, and
...
SlabCity~\cite{dong2023slabcity} is a whole-query, synthesis-based SQL rewriter that searches
...
GenRewrite~\cite{genrewrite} targets the same single-query setting but replaces
...
DAGSmith~\cite{dagsmith-arxiv} lifts these ideas from single queries to pipelines of
