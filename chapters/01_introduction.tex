...
efficient execution strategies is exactly what they are for. But an
optimizer operates within a boundary that is easy to overlook: it selects
the best execution plan for the query it is given, not the best query its
author could have written. Two semantically identical queries, phrased
differently, can lead the same optimizer to plans whose running times
differ by orders of magnitude. When the given phrasing is poor, the
remedy must operate on the query itself. That remedy is query rewriting:
transforming a query into a semantically equivalent form that the
optimizer can turn into a faster plan.
...
its own. As queries grow toward the size and intricacy of
machine-generated BI workloads, the space expands faster than any
structural scoring can tame, and the guidance the search needs stops
looking like a scoring function and starts looking like experience.
...
they are doing. What they do not offer is trust. A language model will
confidently propose rewrites that are subtly inequivalent, rewrites that
are equivalent but no faster, and rewrites whose cost in model queries
...
the insight behind it into a Natural Language Rewrite Rule, a
human-readable strategy stored in a growing repository. These rules
serve three roles at once: they guide the model when new queries
arrive, they explain each suggested rewrite in terms a reviewer can
check, and they carry optimization knowledge from one query to the
next, so the system becomes more capable with every query it
optimizes. When a suggested rewrite turns out to be incorrect, a
counterexample-guided correction loop repairs its syntactic and
semantic errors rather than discarding it, salvaging rewrites that
naive prompting would lose.
...
Beyond the systems themselves, this dissertation contributes reusable
artifacts to the community, including the curated benchmark of
real-life queries constructed for SlabCity's evaluation.
% TODO: add artifact/code links (GitHub footnotes), matching the
% style of the sample dissertations.
