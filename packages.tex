...
%--- Fonts ---------------------------------------------------------
\usepackage[T1]{fontenc} % proper glyphs for < > | in text (papers relied on acmart loading this)
...
% The class routes \url through \href, which href-ul renders as an
% unbreakable underlined box; long bibliography URLs then overflow into
% the margin. Redefine \url to a directly hyperlinked, breakable form
% (colored by urlcolor, not underlined).
\makeatletter
\renewcommand{\url}[1]{\hyper@linkurl{\nolinkurl{#1}}{#1}}
\makeatother
